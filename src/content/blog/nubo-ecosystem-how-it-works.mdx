...
That is the design philosophy behind Nubo: progressive reduction. Manual logging where precision matters. Intelligent automation everywhere else. Three input surfaces feeding into one data layer so every caregiver sees the same picture, whether they tapped a button, spoke to an Echo, or opened their phone.
...
The glue between all three input surfaces is the shared caregiver timeline. When one parent logs a diaper change on the device, the other parent sees it in their app within seconds. When the nanny says "Alexa, tell Newborn Tracker he ate two ounces," both parents see the event appear in their timeline, tagged with the nanny's name.
...
Here is what a typical day looks like when all three surfaces are in use.
...
Three input methods. One data layer. You picked the right tool for each moment without thinking about it. That is what invisible tracking looks like.
