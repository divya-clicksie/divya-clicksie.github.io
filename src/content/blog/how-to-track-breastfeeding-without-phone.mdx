...
Some apps have addressed the biggest pain points. Nubo's Live Activities put a nursing timer directly on the lock screen with a visible left/right side switch, so you never open the app itself. Phone Hand mode shifts the action buttons to whichever side your thumb can reach, left or right. And [Midnight Ember](/blog/product/midnight-ember-night-mode/) replaces the full-spectrum display with an amber-red palette that does not suppress melatonin during nighttime checks. These features reduce the cost of using the phone at 3am. But they do not eliminate the core issue: reaching for the phone in the first place.
